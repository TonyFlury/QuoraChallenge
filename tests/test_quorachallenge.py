...
""",s)
    ...
    @patch.object(_core.requests, 'get', patched_requests(200, test_data=[{'test_id':'001','arguments': '1,2', 'return': '3'},
                                                          ...
                                                          description='This is the description'))
    def test_000_020_multiple_pass(self):
        """Ensure that the multiple tests and a single requested test are correctl recorded"""

        def func(a, b):
            return a + b

        solver = qc.autotest(challenge_name='challenge1', test_id='002',defer_results=True)
    ...
    def test_200_020_dict_extra_key(self):
        ...
        self.assertEqual(len(solver.errors),2)
        self.assertRegex(solver.errors[0], r'Test 0 - Incorrect result : return has these extra keys : 2')
        self.assertRegex(solver.errors[1], r'Test 1 - Incorrect result : return has these extra keys : 3,4')
...
@click.command()
@click.option('-v', '--verbose', default=2, help='Level of return', count=True)
